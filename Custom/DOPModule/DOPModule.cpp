...
void Backtrack::PrintInstructionStack()
{
    ...
    for (auto inst_pair: instList)
    {
        ...
        for (auto tmpV=inst_pair.first.rbegin(); tmpV!=inst_pair.first.rend(); tmpV++)
        {
            errs() << **tmpV << "\n";
        }
    }
...
std::vector<inst_pair_t> Backtrack::GetInstList()
{
    ...
    while (!worklist.empty())
    {
        ...
        else if (dyn_cast<Constant>(ptr))
        {
            ...
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(ptr))
            {
                switch (CE->getOpcode())
                {
                    ...
                    case Instruction::Sub:
                        for (User::op_iterator opi = CE->op_begin(); opi != CE->op_end(); opi++)
                        {
            ...
            else if (dyn_cast<GlobalVariable>(ptr))
            {
                ...
                instList.push_back(inst_pair_t(instruction, value_pair_t(GLOBAL_VAR, ptr)));
                result = true;
            }
            ...
            {
                instList.push_back(inst_pair_t(instruction, value_pair_t(FUNCTION_PTR, ptr)));
                result = true;
            }
        ...
        */
        ...
        {
            // TODO: possible to check all callsites of the function and look for interesting
            // pointers...
    ...
    for (inst_pair_t inst_pair: target.instList)
    {
        ...
        for (auto tmpV=inst_pair.first.rbegin(); tmpV!=inst_pair.first.rend(); tmpV++)
        {
            ...
            else if(BinaryOperator *BO = dyn_cast<BinaryOperator>(*tmpV))
            {
                ...
                // based on llvm 3.5 llvm/IR/Instruction.def
                if (op >= 8 && op <= 19)
...
ResultVal_t InstInst::getPtrSet()
{
    return ptr;
}

ResultVal_t InstInst::getValueSet()
{
    return value;
}
...
{
    errs() << "Global\t:" << target.global << "\n";
    errs() << "Mixed\t:" << target.mixed << "\n";
    errs() << "Local\t:" << target.local << "\n";
}
...
void DOPModule::printStats()
{
    ...
    errs() << "MOV Gadget\n";
    printStat(mov_stats);
    errs() << "Arithmetic Gadgets\n";
    printStat(arithmetic_stats);
    errs() << "Dereference Gadgets\n";
    printStat(deref_stats);
    errs() << "Logical Gadgets\n";
    printStat(logical_stats);
    errs() << "Conditional Gadgets\n";
    ...
    for (auto &F: M)
        for (auto &BB: F)
            for (auto &I: BB)
            {
                ...
                {
                    for (unsigned int x=0; x<TI->getNumSuccessors(); x++)
                    {
                        Instruction *next = &TI->getSuccessor(x)->front();
                        closure_map[&I][next] = 1;
                    }
                }
                else
                {
                    Instruction *next = ((BasicBlock::iterator )(&I))++;
...
bool DOPModule::runOnModule(Module &M)
{
    ...
    if (vuln_path.compare(""))
    {
        ...
        //TODO: memoization can be used to optimize this code to remove redundant traversing.
        for (std::string func: vulnFuncList)
        {
            ...
            while (!worklist.empty())
            {
                ...
                // iterate through the entire user list (callsites that call the function)
                // identifying the function each call function is and adding it back into
                // the worklist. we will be keeping track of the visited functions to 
                // deal with cyclic calls
            ...
            // visited contains all the functions that _MAY_ directly or indirectly call
            // our vulnerable function F
            errs() << visited.size() << "\n";
        }
        exit(1);
    }
...

    {
        ...
        errs() << "\n";
        errs() << "Interesting/Total Loops : ";
        errs() << interesting_loop << "/" << total_loop <<"\n";
        errs() << "\n";
